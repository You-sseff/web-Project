...
$date_conf = date('Y-m-d H:i:s');
                        ...
                        $sql = "INSERT INTO candidat (nom , prenom , date_naiss, telephone, email, cv, id_rh,date_conf) VALUES ('$nom', '$prenom', '$D',  '$tel','$email','$cv','$id','$date_conf')";
                        $res = mysql_query($sql);

					if ($res) {
					echo 'Insertion reussie';

					$chemin = "uploads/candidatures/$id/";
...
$result = $mysqli->query("SELECT id_cand FROM candidat ORDER BY id_cand DESC LIMIT 1");
$row = $result->fetch_assoc();
$id_cand=$row['id_cand'];
$extension = pathinfo($nom_cv, PATHINFO_EXTENSION);
$nouveau_nom = strtolower($prenom . "_" . $nom . "_" . $id_cand . "."  . $extension);  
...
$sqlCv = "UPDATE  candidat SET cv='$cvUrl' WHERE id_cand='$id_cand'";
...
$requete = $mysqli->query("SELECT date_conf FROM candidat ORDER BY date_conf DESC LIMIT 1");
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $email = $_POST['email'];
